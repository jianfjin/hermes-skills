...
point Claude Desktop / Cursor at `tools/mcp_tool.py` — that file has no server
object. If you need Hermes to *be* an MCP server (expose its data to other
clients), use `hermes mcp serve` or write a standalone FastMCP script. See the
`hermes-agent` skill reference `migration-and-mcp.md` for a working KB-server
template.
